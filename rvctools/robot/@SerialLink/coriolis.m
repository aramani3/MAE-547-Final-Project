...
function C = coriolis(robot, q, qd)
    ...
    robot2.fast=0;%added by Aditya Ramani
    ...
    if isa(q, 'sym')
        C = sym(zeros(N,N));%changed by Aditya Ramani from C(N,N)=sym() to correct syntax error
        Csq = sym(zeros(N,N));%changed by Aditya Ramani from Csq(N,N)=sym() to correct syntax error
    else
